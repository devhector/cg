...
let rotationSpeed = degToRad(1); // Velocidade de rotação (1 grau convertido para radianos)
...
// ---------------------------------------
  ...
  const obj = parseOBJ(loadFile('assets/building_G.obj').text());
  const materials = parseMTL(loadFile('assets/building_G.mtl').text());
  ...
  async function loadFile(filePath) {
    ...
    if (!response.ok) {
      throw new Error(`Não foi possível carregar o arquivo ${filePath}: ${response.statusText}`);
    }
